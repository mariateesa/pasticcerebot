...
async def cerca(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if trovate:
        ...
        await update.message.reply_text(
            f"Nessuna ricetta con '{ingrediente}' trovata."
        )
...
async def gestisci_messaggio(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Manda messaggio iniziale che verrà aggiornato
    msg = await update.message.reply_text("...")
    queue: asyncio.Queue = asyncio.Queue()

    # Ollama gira in un thread separato e mette i token nella queue
    def ollama_thread():
        for token in rispondi_stream(domanda, chat_id):
            loop.call_soon_threadsafe(queue.put_nowait, token)
        loop.call_soon_threadsafe(queue.put_nowait, None)  # segnale di fine

    threading.Thread(target=ollama_thread, daemon=True).start()

    # Legge i token dalla queue e aggiorna il messaggio su Telegram
    testo = ""
    ultimo_aggiornamento = time.time()
    INTERVALLO = 1.5  # secondi tra un edit e l'altro (limite Telegram)
    ...
    # Aggiornamento finale con il testo completo
    if testo.strip():
        try:
            await msg.edit_text(testo)
        except Exception:
